...
public class CatController : MonoBehaviour
{
    ...
    private void SetPatrolMovement()
    {
        ...
        {
            if (!_isWaiting)//hala halihazırda beklemiyorsam
            {
                _isWaiting = true;
                _timer = _waitTime;
    ...
    private void SetRandomDestination()
    {
        ...
        while (attempts < _maxDestinationAttempts && !destinationSet)
        {
            ...
            if (NavMesh.SamplePosition(randomDirection, out NavMeshHit hit, _patrolRadius, NavMesh.AllAreas))//raycast mantığı
            {
                Vector3 finalPosition = hit.position;
                if (!IsPositionBlocked(finalPosition))
                {
                    _catAgent.SetDestination(finalPosition);//navmeshden gelen özellik
                    destinationSet = true;
                }
                else
                {
        ...
        if (!destinationSet)
        {
            Debug.LogWarning("Could not find a valid patrol destination after multiple attempts. Returning to initial position.");
            _isWaiting = true;
            _timer = _waitTime * 2;
        }
    }
