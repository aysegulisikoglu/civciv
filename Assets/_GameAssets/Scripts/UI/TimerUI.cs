...
public class TimerUI : MonoBehaviour
{
    ...
    private void PlayRotationAnimation()
    {
        _rotationTween = _timerRotatableTransform.DORotate(new Vector3(0f, 0f, -360f), _rotationDuration, RotateMode.FastBeyond360)
              .SetLoops(-1, LoopType.Restart)//sınırsız istiyosan -1
    ...
    private void StartTimer()
    {
        ...
        _elapsedTime = 0f;
        InvokeRepeating(nameof(UpdsateTimerUI), 0f, 1f);
    }
    private void StopTimer()
    {
        _isTimerRunning = false;
        CancelInvoke(nameof(UpdsateTimerUI));
